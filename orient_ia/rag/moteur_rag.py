...
class MoteurRAG:
    ...
    def rechercher(self, requete: str, top_k: int = 4, seuil_min: float = 0.05) -> List[ResultatRecherche]:
        ...
        # Scoring hybride avec bonus pour correspondance exacte de mots clés (acronymes, mentions)
        mots_cles = set(req_norm.split())
        ...
        for idx, passage in enumerate(self.passages):
            ...
            # Bonus de correspondance lexicale sur les sigles clés (ex. ISAIA, IGGLIA, BACC, etc.)
            intersection = mots_cles.intersection(mots_passage)
            bonus_mots = len(intersection) * 0.08
